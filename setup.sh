...
# Pulse speakers only ever use A2DP for music. Restrict the bluez roles to A2DP
# (plus LE-audio BAP) and disable the HFP backend, so WirePlumber never
# advertises or chases a Hands-Free profile on an offline/absent speaker: that
# retry loop fires every ~60s, grabs the shared 2.4GHz radio (Wi-Fi + BT on one
# chip), and stalls Wi-Fi long enough to break snapcast time-sync.
...
write_backlight_conf() {
    local default_lat="37.203160"
    local default_lon="-79.973197"
    local lat="$default_lat"
    local lon="$default_lon"
    local day_brightness="${PULSE_DAY_BRIGHTNESS:-85}"
    local night_brightness="${PULSE_NIGHT_BRIGHTNESS:-25}"
    local twilight="${PULSE_TWILIGHT_MODE:-OFFICIAL}"
    local backlight="${PULSE_BACKLIGHT_DEVICE:-/sys/class/backlight/11-0045}"

    local resolved
    resolved=$(
        PYTHONPATH="$REPO_DIR" \
        PULSE_LOCATION="${PULSE_LOCATION:-}" \
        PULSE_WEATHER_LANGUAGE="${PULSE_WEATHER_LANGUAGE:-en}" \
...
PY
    ...
    sudo tee /etc/pulse-backlight.conf >/dev/null <<EOF
# Generated by setup.sh — updates should come from pulse.conf or MQTT
LAT=${lat}
LON=${lon}
